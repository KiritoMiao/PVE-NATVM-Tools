...
def set_nat_rule(protocol, external_port, internal_ip, internal_port):
    if check_nat_rule(protocol, external_port, internal_ip, internal_port) == 0:
        result = os.system(
            "/usr/sbin/iptables -A PREROUTING -t nat -p " + protocol + " -m " + protocol + " -i " + PUBLIC_IF + " --dport " + external_port + " -j DNAT --to-destination " + internal_ip + ":" + internal_port)
        if result == 0:
            # print("success")
            return 0
        else:
...
def del_nat_rule(protocol, external_port, internal_ip, internal_port):
    if check_nat_rule(protocol, external_port, internal_ip, internal_port) == 1:
        result = os.system(
            "/usr/sbin/iptables -D PREROUTING -t nat -p " + protocol + " -m " + protocol + " -i " + PUBLIC_IF + " --dport " + external_port + " -j DNAT --to-destination " + internal_ip + ":" + internal_port)
        if result == 0:
            # print("success")
            return 0
        else:
